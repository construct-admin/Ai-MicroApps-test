...
st.caption(
    "Two modes: Local (uses your Chrome profile) or Cloud (headless Chromium via CAUTH cookie).\n"
    "Agent traverses Coursera /teach Shadow DOM, clicks each item, extracts visible text (with DOM‑selection fallback),"
    " then exports a .docx ready for Google Docs."
)
...
def ensure_playwright():
    ...
    try:
        ...
        try:
            ...
            st.stop()
    try:
        # Ensure Chromium is installed (no‑op if already present)
        subprocess.run([sys.executable, "-m", "playwright", "install", "chromium"], check=True)
    except Exception:
        # Ignore if already present; Cloud may have it via packages.txt image
        pass
...
# ======================================================
# Shadow‑DOM helpers (run inside the page)
# ======================================================
          ...
          for (const m of moduleRows) {
            ...
            uniqueItems.forEach((it, idx) => {
              ...
              const titleCand = leafKids.find(a => a.getAttribute && (
                ...
                a.tagName === "H4" || a.tagName === "H5" ||
                a.tagName === "A" || a.tagName === "DIV"
              )) || it;
    ...
    with sync_playwright() as p:
        try:
            ...
            result.errors.append(
                "Failed to open Chrome profile. Close all Chrome windows and retry. Details: " + str(e)
            )
            return result
...
- Repo root **requirements.txt** (no Playwright here):
```
streamlit>=1.36
python-docx>=1.1
pandas>=2.2
```
- Repo root **packages.txt** (Chromium libs):
```
libnss3
libnspr4
libatk1.0-0
...
libxcb1
```
- (Optional) **runtime.txt**: `python-3.11`
- After pushing changes, **Manage app → Clear cache & reboot**.

**Notes**
- Cloud mode needs a valid Coursera **CAUTH** cookie to read the /teach editor.
- If Coursera UI shifts, tweak the selectors in `deep_query_eval_script()`.
- DOM‑selection fallback avoids OS clipboard and works headless.
    """
)