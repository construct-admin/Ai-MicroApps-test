...
--------------------------
A Streamlit-based micro-application used to:

1. Parse storyboard-style DOCX files or Google Docs exports.
2. Extract <canvas_page> and related tag-based content blocks.
3. Transform that content into Canvas-ready HTML (and Quiz JSON for quizzes)
   via GPT (OpenAI gpt-4o / gpt-4o-mini).
4. Optionally leverage Knowledge Base (Vector Store) template retrieval.
5. Upload the generated HTML, assignments, quizzes, or discussions to a
   Canvas LMS course using the Canvas REST API.

This refactor introduces:
    - Unified, professional OES GenAI UX (layout, grouping, naming).
    - Sidebar-driven configuration (Canvas, OpenAI, KB, global toggles).
    - Structured functions and maintainable grouping of UI sections.
    - Behavior-preserving rewrite: **no functional changes**.
    - 100% preservation of base_rules (GPT instruction block) — byte-for-byte.
    - Consistent code style, import hygiene, and readability upgrade.
...
Dependencies:
    ...
    - fitz or PyPDF2 (not used in this project, but common in OES stack)

External APIs:
    - Canvas LMS API v1
    - OpenAI GPT-4o / GPT-4o-mini Models
    - Google Drive (optional DOCX export)
...
from gdoc_utils import (
    gdoc_id_from_url,
    fetch_docx_from_gdoc,
)

# Storyboard module-tag parsing
from module_tags import split_text_by_module_tags

# Canvas-page parsers
from parsers import (
    extract_canvas_pages_from_text,
    extract_canvas_pages,
    scan_canvas_page_tags,
)
...
from quizzes_new import add_new_quiz, add_item_for_question
...
def main():
    st.set_page_config(
        page_title="📄 DOCX → GPT (KB / Course Templates) → Canvas", layout="wide"
    )
    st.title(
        "📄 Upload DOCX → Convert via GPT (KB / Course Templates) → Upload to Canvas"
    )
    ...
    with st.expander("📝 Storyboard Settings", expanded=True):
        ...
        with sb_col3:
            sa_json = st.file_uploader(
                "Service Account JSON (for Drive read, optional)", type=["json"]
            )
        ...
        def _read_entire_doc_as_text() -> str:
            ...
            try:
                ...
                st.error(
                    "python-docx is required. Add `python-docx` to requirements.txt."
                )
                return ""
        ...
        with scan_col:
            if st.button(
                "🔎 Scan for <module_name>…</module> tags", use_container_width=True
            ):
                raw_text = _read_entire_doc_as_text()
                if not raw_text.strip():
                    st.warning(
                        "No storyboard loaded yet (upload a .docx or provide a GDoc URL + SA JSON)."
                    )
                else:
        ...
        with pick_col:
            ...
            if mods:
                ...
                if label and label != "(pick a module)":
                    ...
                    st.info(
                        f"Selected: **{chosen['name']}**  — {len(chosen['text'])} chars"
                    )
    ...
    with st.expander("📚 Knowledge Base (Vector Store) — optional", expanded=False):
        ...
        with kb1:
            existing_vs = st.text_input(
                "Vector Store ID", value=st.session_state.get("vector_store_id") or ""
            )
        with kb2:
        ...
        with btns[0]:
            if st.button(
                ...
                disabled=not (kb_client and kb_supported),
            ):
                try:
        ...
        with btns[1]:
            if st.button(
                ...
                use_container_width=True,
                disabled=not (
                    (st.session_state.get("vector_store_id") or existing_vs)
                    and kb_client
                    and kb_supported
                ),
            ):
                try:
                    ...
                    if kb_docx is not None:
                        ...
                        if fid:
                            data = fetch_docx_from_gdoc(
                                fid, st.session_state["_sa_bytes"]
                            )
                    ...
                    if not vs_id:
                        ...
                        st.error(
                            "Provide a template .docx or Google Doc URL + SA JSON."
                        )
                    else:
                        data, fname = got
                        ...
                        if status == "completed":
                            ...
                            st.error(
                                f"Upload error ({via}): {res.get('error', 'unknown')}"
                            )
                except Exception as e:
        ...
        with btns[2]:
            if st.button("Use Existing VS ID", use_container_width=True):
                if existing_vs.strip():
                    ...
                    st.success(
                        f"✅ Using Vector Store: {st.session_state.vector_store_id}"
                    )
                else:
    ...
    with st.expander("🎓 Canvas Credentials & Course Structure", expanded=True):
        ...
        with can1:
            canvas_domain = st.text_input(
                "Canvas Base URL", placeholder="youruni.instructure.com"
            )
        ...
        if st.button(
            ...
            try:
                ...
                st.session_state.course_modules = [
                    {"id": m["id"], "name": m["name"]} for m in mods
                ]
    ...
    with st.expander("🤖 OpenAI API Credentials", expanded=True):
        ...
        with c1:
            st.session_state["gpt_model"] = st.selectbox(
                "Model", ["gpt-4o", "gpt-4o-mini"], index=0
            )
        with c2:
            st.session_state["gpt_max_tokens"] = st.number_input(
                "Max output tokens", min_value=256, max_value=8192, value=2000, step=128
            )
        with c3:
            st.session_state["gpt_throttle"] = st.number_input(
                "Throttle per call (sec)",
                min_value=0.0,
                max_value=2.0,
                value=0.4,
                step=0.1,
    ...
    with parse_cols[0]:
        ...
        if st.button(
            "1️⃣ Parse storyboard",
            type="primary",
            use_container_width=True,
            disabled=not has_story,
        ):
            ...
            if tag_text:
                ...
                st.caption(
                    f"Canvas-page tags → start: {diag['starts']}  "
                    f"end: {diag['ends']}  balanced: {diag['balanced']}"
                )
            ...
            for idx, block in enumerate(raw_pages):
                ...
                page_title = (
                    extract_tag("page_title", block) or f"Page {idx+1}"
                ).strip()
                module_name = (
                    extract_tag("module_name", block) or last_known_module or "General"
                ).strip()
            ...
            st.success(
                f"✅ Parsed {len(st.session_state.pages)} item(s) from '{tag_name}'."
            )
    ...
    if st.session_state.pages:
        ...
        module_options = ["(pick module)"] + [
            m["name"] for m in st.session_state.course_modules
        ]
        TYPE_OPTIONS = ["page", "assignment", "discussion", "quiz"]
        ...
        for i, p in enumerate(st.session_state.pages):
            ...
            with st.expander(header, expanded=False):
                ...
                with c1:
                    p["page_title"] = st.text_input(
                        "Title", value=p["page_title"], key=f"title_{i}"
                    )
                ...
                with c2:
                    p["module_name"] = st.text_input(
                        "Target Module Name", value=p["module_name"], key=f"module_{i}"
                    )
                    if module_options and len(module_options) > 1:
                        pick_name = st.selectbox(
                            "…or pick existing module",
                            module_options,
                            key=f"modpick_{i}",
                        )
                    ...
                    if tm_pick and tm_pick != "(pick module)":
                        ...
                        if mod_id:
                            if mod_id not in st.session_state.module_pages_cache:
                                items = list_module_items(
                                    canvas_domain, course_id, mod_id, canvas_token
                                )
                                ...
                                st.session_state.module_discussions_cache[mod_id] = [
                                    {
                                        "title": it.get("title"),
                                        "id": it.get("content_id"),
                                    }
                                    for it in items
                                    if it.get("type") == "Discussion"
                                    and it.get("content_id")
                                ]
                            ...
                            if p["page_type"] == "page":
                                page_opts = ["(pick page)"] + [
                                    x["title"]
                                    for x in st.session_state.module_pages_cache.get(
                                        mod_id, []
                                    )
                                ]
                                with tm_cols[1]:
                                    page_pick = st.selectbox(
                                        "Template Page", page_opts, key=f"tmpl_page_{i}"
                                    )
                                if page_pick and page_pick != "(pick page)":
                                    page_url = next(
                                        (
                                            x["page_url"]
                                            for x in st.session_state.module_pages_cache[
                                                mod_id
                                            ]
                                            if x["title"] == page_pick
                                        ),
                                        None,
                                    )
                                    if page_url:
                                        html, _ = get_page_body(
                                            canvas_domain,
                                            course_id,
                                            page_url,
                                            canvas_token,
                                        )
                                        st.session_state.per_item_course_template_html[
                                            i
                                        ] = (html or "")
                                ...
                                disc_opts = ["(pick discussion)"] + [
                                    x["title"]
                                    for x in st.session_state.module_discussions_cache.get(
                                        mod_id, []
                                    )
                                ]
                                with tm_cols[1]:
                                    disc_pick = st.selectbox(
                                        "Template Discussion",
                                        disc_opts,
                                        key=f"tmpl_disc_{i}",
                                    )
                                if disc_pick and disc_pick != "(pick discussion)":
                                    did = next(
                                        (
                                            x["id"]
                                            for x in st.session_state.module_discussions_cache[
                                                mod_id
                                            ]
                                            if x["title"] == disc_pick
                                        ),
                                        None,
                                    )
                                    if did:
                                        html, _ = get_discussion_body(
                                            canvas_domain, course_id, did, canvas_token
                                        )
                                        st.session_state.per_item_course_template_html[
                                            i
                                        ] = (html or "")
                                ...
                                q_opts = ["(pick classic quiz)"] + [
                                    x["title"]
                                    for x in st.session_state.module_quizzes_cache.get(
                                        mod_id, []
                                    )
                                ]
                                a_opts = ["(pick assignment)"] + [
                                    x["title"]
                                    for x in st.session_state.module_assignments_cache.get(
                                        mod_id, []
                                    )
                                ]
                                with tm_cols[1]:
                                    quiz_pick = st.selectbox(
                                        "Template (Classic Quiz)",
                                        q_opts,
                                        key=f"tmpl_quiz_{i}",
                                    )
                                with tm_cols[2]:
                                    asg_pick = st.selectbox(
                                        "Template (Assignment / New Quiz)",
                                        a_opts,
                                        key=f"tmpl_asg_{i}",
                                    )
                                if quiz_pick and quiz_pick != "(pick classic quiz)":
                                    qid = next(
                                        (
                                            x["id"]
                                            for x in st.session_state.module_quizzes_cache[
                                                mod_id
                                            ]
                                            if x["title"] == quiz_pick
                                        ),
                                        None,
                                    )
                                    if qid:
                                        desc, _ = get_quiz_description(
                                            canvas_domain, course_id, qid, canvas_token
                                        )
                                        st.session_state.per_item_course_template_html[
                                            i
                                        ] = (desc or "")
                                        st.success(
                                            "Loaded classic-quiz template description."
                                        )
                                elif asg_pick and asg_pick != "(pick assignment)":
                                    aid = next(
                                        (
                                            x["id"]
                                            for x in st.session_state.module_assignments_cache[
                                                mod_id
                                            ]
                                            if x["title"] == asg_pick
                                        ),
                                    ...
                                    if aid:
                                        desc, _ = get_assignment_description(
                                            canvas_domain, course_id, aid, canvas_token
                                        )
                                        st.session_state.per_item_course_template_html[
                                            i
                                        ] = (desc or "")
                                        st.success(
                                            "Loaded assignment template description."
                                        )
    ...
    if st.session_state.pages:
        ...
        if st.button(
            ...
            for idx in selected_indices:
                ...
                # DO NOT CHANGE THIS BLOCK (base_rules) – kept exactly as provided
                base_rules = (
                ...
                if template_html:
                    ...
                    SYSTEM = base_rules + (
                        "\nUse file_search to locate the best matching template if available. Return HTML only."
                        if tools
                        else ""
                    )
    ...
    if st.session_state.pages and st.session_state.visualized:
        ...
        with global_upload_btn_cols[0]:
            do_global_upload = st.button(
                "🚀 Upload ALL Selected (across tabs)", type="secondary", disabled=False
            )

        module_cache = {}

        def _upload_item(p, html_result, quiz_json):
            mid = get_or_create_module(
                p["module_name"], canvas_domain, course_id, canvas_token, module_cache
            )
            if not mid:
            ...
            if p["page_type"] == "page":
                page_url = add_page(
                    canvas_domain, course_id, p["page_title"], html_result, canvas_token
                )
                return bool(
                    page_url
                    and add_to_module(
            ...
            if p["page_type"] == "quiz":
                ...
                if (
                    quiz_json
                    and isinstance(quiz_json, dict)
                    and "quiz_description" in quiz_json
                ):
                    description = quiz_json.get("quiz_description") or html_result

                if use_new_quizzes:
                    q_list = (
                        (quiz_json or {}).get("questions", [])
                    ...
                    if unsupported:
                        ...
                        if qid:
                            for q in q_list:
                                add_quiz_question(
                                    canvas_domain, course_id, qid, q, canvas_token
                                )
                            return add_to_module(
                                canvas_domain,
                                course_id,
                                mid,
                                "Quiz",
                        ...
                        for pos, q in enumerate(q_list, start=1):
                            ...
                            if not ok:
                                st.warning(
                                    f"Failed to add item {pos} ({q.get('question_type')}): {dbg}"
                                )
                        ...
                        if not ok:
                            st.warning(
                                "Created New Quiz but failed to add it to the module."
                            )
                        return ok
        ...
        for tab_idx, tab in enumerate(tabs):
            ...
            with tab:
                items = [
                    p for p in st.session_state.pages if p["page_type"] == target_type
                ]
                ...
                with tcols[2]:
                    do_tab_upload = st.button(
                        ...
                        disabled=dry_run
                        or not (canvas_domain and course_id and canvas_token),
                    )
                ...
                for p in items:
                    ...
                    with st.expander(meta, expanded=False):
                        ...
                        quiz_json = st.session_state.gpt_results.get(idx, {}).get(
                            "quiz_json"
                        )
                        ...
                        can_upload = (not dry_run) and (
                            canvas_domain and course_id and canvas_token
                        )
                        if st.button(
                            f"Upload '{p['page_title']}'",
                            key=f"upl_{idx}",
    ...
    if not st.session_state.get("selected_tag_module_text"):
        st.info(
            "Scan for `<module_name>…</module>` tags and pick a module, then click **Parse storyboard**.",
            icon="📝",
        )
    elif (
        st.session_state.get("selected_tag_module_text") and not st.session_state.pages
    ):
        st.warning(
            "Click **Parse storyboard** to extract the items in the chosen module.",
            icon="👉",
        )
    elif st.session_state.pages and not st.session_state.visualized:
        st.info(
            "Review & adjust metadata above (and pick course templates if desired), then click **Visualize selected**.",
            icon="🔎",
        )
